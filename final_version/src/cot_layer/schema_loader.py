...
def load_slot_schemas(path: str) -> list[SlotSchema]:
    ...
    for raw_line in file_path.read_text(encoding="utf-8").splitlines():
        ...
        metadata["slot_terms"] = extract_slot_terms(slot_term=slot_term, metadata=metadata)
        controlled_vocabulary = extract_controlled_vocabulary(
            slot_term=slot_term,
            description=description,
            slot_terms=metadata["slot_terms"],
        )
        items.append(
            SlotSchema(
                slot_name=slot_name,
...
def extract_slot_terms(slot_term: str, metadata: dict | None = None) -> list[str]:
    metadata = metadata if isinstance(metadata, dict) else {}
    raw_terms = metadata.get("slot_terms", [])
    candidates: list[str] = []
    if slot_term.strip():
        candidates.append(slot_term.strip())
    if isinstance(raw_terms, list):
        candidates.extend(str(item).strip() for item in raw_terms if str(item).strip())
    return _dedupe_slot_terms(candidates)


def extract_controlled_vocabulary(slot_term: str, description: str, slot_terms: list[str] | None = None) -> list[str]:
    candidates: list[str] = []
    if isinstance(slot_terms, list):
        candidates.extend(str(item).strip() for item in slot_terms if str(item).strip())
...
def _dedupe_slot_terms(candidates: list[str]) -> list[str]:
    results: list[str] = []
    seen: set[str] = set()
    for candidate in candidates:
