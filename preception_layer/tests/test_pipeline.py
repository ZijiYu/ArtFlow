...
def test_llm_chat_record_is_refreshed_on_each_run() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        first_run_lines = (root / "llm_chat_record.jsonl").read_text(encoding="utf-8").strip().splitlines()

        asyncio.run(pipeline.run(image_file=image_path, input_text="第二次运行"))
        second_run_lines = (root / "llm_chat_record.jsonl").read_text(encoding="utf-8").strip().splitlines()

    assert first_run_lines
    assert second_run_lines
    assert len(second_run_lines) == len(first_run_lines)
    assert "第二次运行" in second_run_lines[0]
    assert "第一次运行" not in "\n".join(second_run_lines)
...
def test_downstream_prompt_runner_preserves_existing_llm_chat_record() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        final_text = (root / "llm_chat_record.jsonl").read_text(encoding="utf-8")

    assert response["status"] == "ok"
    assert len(first_run_lines) >= 1
    assert "主流程运行" in final_text
    assert "请补充两个更细的问题" in final_text
